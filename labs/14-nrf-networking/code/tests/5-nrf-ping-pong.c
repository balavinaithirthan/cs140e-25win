...
ping_pong_ack(nrf_t *s, nrf_t *c, int verbose_p) {
    ...
    for(unsigned i = 0; i < ntrial; i++) {
        ...
        npackets++;
    // while(1) {
    //     while (net_put32(s, client_addr, ++exp) != 4) {

    //     }
    //     if(!net_get32(s, &got_recv))
    //         ntimeout++;            
    //     else {
    //         nrf_output("server: received %d\n", got);
    //         npackets++;
    //         exp++;
    //     }
    //     if (exp == 1000) {
    ...
    // }
    // while (1) {
    //     while(!net_get32(c, &got)) { // server: 0x10, client 0x05
    //         ;
    //     }
    //     net_put32(c, server_addr, ++got);
    // }
    //     if(!net_get32(s, &got))
    //         ntimeout++;
    //     else if(got != exp)
