...
static unsigned gpio_fsel0_v, gpio_fsel1_v, gpio_fsel2_v, gpio_fsel3_v,
    // do a hack to set initial value: don't use random.
    gpio_fsel4_v = ~0, gpio_set0_v, gpio_clr0_v, gpio_set1_v, gpio_clr1_v;
...
void delay_cycles(unsigned ncycles) { trace("delaying %d cycles\n", ncycles); }

void delay_us(unsigned usec) { trace("delaying %d usec\n", usec); }
void delay_ms(unsigned msec) { trace("delaying %d msec\n", msec); }
...
void uart_flush_tx(void) { fflush(stdout); }
